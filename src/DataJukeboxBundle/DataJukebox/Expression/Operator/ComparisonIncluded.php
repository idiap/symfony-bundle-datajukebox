...
// DataJukeboxBundle\DataJukebox\Expression\Operator\ComparisonIncluded.php
...
/** Comparison: included (in)
 *
 * @package    ExpressionLanguage
 * @subpackage Operator
 */
class ComparisonIncluded
  extends Expression\Operator
{
  ...
  {
    return sprintf('%s in %s', $oLeft->toString($bCompact), $oRight->toString($bCompact));
  }
  ...
  public function evaluate(Expression\Node $oLeft, Expression\Node $oRight, array $amParameters)
  {
    ...
    if (!is_scalar($mLeft)) {
      throw new Expression\Exception(sprintf('[%s] Operand must be scalar (L:%s)', get_class($this), $oLeft));
    }
    $aRight = $oRight->evaluate($amParameters);
    if (!is_array($aRight)) $aRight = array($aRight);
    return in_array($mLeft, $aRight);
  }
