...
// DataJukeboxBundle\DataJukebox\Expression\Operator\ArithmeticAddition.php
  ...
  extends Expression\Operator
{
  ...
  public function evaluate(Expression\Node $oLeft, Expression\Node $oRight, array $amParameters)
  {
    ...
    if (!is_numeric($mLeft) or !is_numeric($mRight)) {
      throw new Expression\Exception(sprintf('[%s] Operands must be numeric (L:%s, R:%s)', get_class($this), $oLeft, $oRight));
    }
    return pow($mLeft, $mRight);
  }
