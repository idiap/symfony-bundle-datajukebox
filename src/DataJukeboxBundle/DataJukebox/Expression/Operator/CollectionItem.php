...
// DataJukeboxBundle\DataJukebox\Expression\Operator\CollectionItem.php
  ...
  extends Expression\Operator
{
  ...
  {
    return sprintf('%s[%s]', $oLeft->toString($bCompact), $oRight->toString($bCompact));
  }
  ...
  public function evaluate(Expression\Node $oLeft, Expression\Node $oRight, array $amParameters)
  {
    ...
    if (!is_array($mLeft)) {
      throw new Expression\Exception(sprintf('[%s] Operand must be an array (L:%s)', get_class($this), $oLeft));
    }
    $mRight = $oRight->evaluate($amParameters);
    if (!is_scalar($mRight)) {
      throw new Expression\Exception(sprintf('[%s] Operand must be scalar (R:%s)', get_class($this), $oRight));
    }
    if (!array_key_exists($mRight, $mLeft)) {
      throw new Expression\Exception(sprintf('[%s] Invalid key (L:%s, R:%s)', get_class($this), $oLeft, $oRight));
    }
    return $mLeft[$mRight];
  }
