...
// DataJukeboxBundle\DataJukebox\Expression\Operator\QuoteDouble.php
  ...
  extends Expression\Operator
{
  ...
  public function evaluate(Expression\Node $oLeft, Expression\Node $oRight, array $amParameters)
  {
    ...
    if (!is_scalar($mRight)) {
      throw new Expression\Exception(sprintf('[%s] Operand must be scalar (%s)', get_class($this), $oRight));
    }
    $sString = '';
    foreach (Expression\Expression::tokenizeStringParameters($mRight) as $aToken) {
      switch ($aToken['type']) {

      case Expression::TOKEN_VALUE:
        $sString .= preg_replace('/""/u', '"', $aToken['value']);
        break;

      case Expression::TOKEN_PARAMETER:
        if (!array_key_exists($aToken['value'], $amParameters)) {
          throw new Expression\Exception(sprintf('[%s] No value for parameter (%s:%s)', get_class($this), $oRight, $aToken['value']));
        }
        $sString .= $amParameters[$aToken['value']];
        break;

      default:
        throw new Expression\Exception(sprintf('[%s] Invalid expression (%s)', get_class($this), $oRight));
      }
    }
    return $sString;
  }
