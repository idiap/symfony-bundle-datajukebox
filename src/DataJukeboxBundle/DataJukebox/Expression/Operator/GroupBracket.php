...
// DataJukeboxBundle\DataJukebox\Expression\Operator\GroupBracket.php
  ...
  extends Expression\Operator
{
  ...
  {
    return sprintf($bCompact ? '[%s]' : '[ %s ]', $oRight->toString($bCompact));
  }

  public function evaluate(Expression\Node $oLeft, Expression\Node $oRight, array $amParameters)
  {
    $aValue = $oRight->evaluate($amParameters);
    if (!is_array($aValue)) $aValue = array($aValue);
    return $aValue;
  }
