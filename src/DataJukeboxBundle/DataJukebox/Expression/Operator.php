...
// DataJukeboxBundle\DataJukebox\Expression\Operator.php
...
abstract class Operator
{
  ...
  /** Returns a string representation of the operator content
   * @param Node $oLeft Left-hand node (operand)
   * @param Node $oRight Right-hand node (operand)
   * @param boolean $bCompact Prefer a compact string representation
   * @return string String representation of the operator content
   */
  abstract public function toString(Node $oLeft, Node $oRight, $bCompact=false);

  /** Returns the evaluated value of the operator content
   * @param Node $oLeft Left-hand node (operand)
   * @param Node $oRight Right-hand node (operand)
   * @param array|mixed $amParameters Array associating parameters name and their value
   * @return mixed Evaluated value of the operator content
   */
  abstract public function evaluate(Node $oLeft, Node $oRight, array $amParameters);
