...
// DataJukeboxBundle\DataJukebox\Filter\Operator\SignNegative.php
 ...
 */
class SignNegative
  extends Expression\Operator\SignNegative
{

  /*
   * METHODS: Expression\SignNegative
   ********************************************************************************/
  ...
  public function evaluate(Expression\Node $oLeft, Expression\Node $oRight, array $amParameters)
  {
    ...
    if (!is_scalar($mRight)) {
      throw new Filter\Exception(sprintf('[%s] Operand must be scalar (%s)', get_class($this), $oRight));
    }
    return sprintf('-%s', $mRight);
  }
