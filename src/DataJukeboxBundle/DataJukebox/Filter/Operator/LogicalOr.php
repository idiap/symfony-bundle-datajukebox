...
// DataJukeboxBundle\DataJukebox\Filter\Operator\LogicalXor.php
 ...
 */
class LogicalOr
  extends Expression\Operator\LogicalOr
{

  /*
   * METHODS: Expression\LogicalOr
   ********************************************************************************/

  public function toString(Expression\Node $oLeft, Expression\Node $oRight, $bCompact=false)
  {
    return sprintf($bCompact ? '%s||%s' : '%s or %s', $oLeft->toString($bCompact), $oRight->toString($bCompact));
  }
  ...
  public function evaluate(Expression\Node $oLeft, Expression\Node $oRight, array $amParameters)
  {
    ...
    return sprintf('%s OR %s', $mLeft, $mRight);
  }
